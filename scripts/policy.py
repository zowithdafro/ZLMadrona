...
def setup_obs(sim):
    ...
    lava_obs_tensor = sim.lava_observation_tensor().to_torch()
    ...
    obs_tensors = [
        ...
        lava_obs_tensor.view(batch_size, *lava_obs_tensor.shape[2:]),
        id_tensor,
    ]
...
def process_obs(self_obs, partner_obs, room_ent_obs,
                door_obs, lidar, steps_remaining, lava_obs, ids):
    ...
    return torch.cat([
        ...
        lava_obs.view(lava_obs.shape[0], -1),
        ids,
    ], dim=1)
