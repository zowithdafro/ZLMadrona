...
static Entity makeLavaEntity(Engine &ctx, Vector3 position){
    
    Entity lava = ctx.makeEntity<LavaEntity>();
    ctx.get<Position>(lava) = position;
    ...
    ctx.get<LavaState>(lava).isDead = false;
                                   ...
                                   float y_max)
{
    ...
    float lava_y = randBetween(ctx, y_min + consts::roomLength / 4.f,
    y_max - consts::wallWidth - consts::lavaHeight);
    //while (abs(lava_y - button_y) < consts::lavaHeight + consts::buttonWidth) {
                                   ...
                                   float y_max)
{
    ...
    do {
        ...
        lava_x = randInRangeCentered(ctx, consts::worldWidth / 2.f - consts::lavaWidth);
        lava_y = randBetween(ctx, y_min + consts::roomLength / 4.f, y_max - consts::wallWidth - consts::lavaHeight);
