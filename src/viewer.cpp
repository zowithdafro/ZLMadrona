...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 0; i < render_asset_paths.size(); i++) {
        ...
        std::cout << render_asset_cstrs[i] << std::endl;
        std::cout << render_asset_paths.size() << std::endl;
    ...
    auto materials = std::to_array<imp::SourceMaterial>({
        ...
        { rgb8ToFloat(255, 0, 0), -1, 0.8f, 0.2f }, //red
    ...
    auto lava_printer = mgr.lavaObservationTensor().makePrinter();
