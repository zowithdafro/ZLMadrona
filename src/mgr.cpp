...
{
    std::cout << "Starting loadPhysicsObjects" << std::endl;
    ...
    asset_paths[(size_t)SimObject::Plane] =
    ...
    std::cout << "Importing assets from disk..." << std::endl;
    ...
    if (!imported_src_hulls.has_value()) {
        std::cerr << "Failed to import assets: " << import_err_buffer << std::endl;
        return; // Or handle the error as needed
    } else {
        std::cout << "Assets imported successfully" << std::endl;
    }
    if (!imported_src_hulls.has_value()) {
    ...
    auto setupHull = [&](SimObject obj_id,
        ...
        std::cout << "Setting up hull for SimObject ID: " << static_cast<int>(obj_id) << std::endl;
        ...
        for (const imp::SourceMesh &mesh : meshes) {
            ...
            std::cout << "Added mesh to convex hulls, current count: " << src_convex_hulls.size() << std::endl;
        }
        std::cout << "All hulls set up successfully" << std::endl;
    ...
    setupHull(SimObject::Lava, 1.f, {
            .muS = 0.5f,  // Modify friction parameters as needed
            .muD = 0.5f,
    });
...
Tensor Manager::lavaObservationTensor() const {
    return impl_->exportTensor(ExportID::LavaObservation, Tensor::ElementType::Float32,
                               {
                                   impl_->cfg.numWorlds,
                                   consts::numAgents,
                                   3,
